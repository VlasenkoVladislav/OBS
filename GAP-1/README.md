...
Развёрнут стек мониторинга через Docker Compose на одной виртуальной машине (Ubuntu).
Prometheus собирает метрики каждые 5 секунд со всех компонентов окружения (  scrape_interval: 5s).

Пользователь для MySQLэкспортера создан вручную в MySQL
Проверка выполнялась в Grafana → Explore (datasource: Prometheus) на UP

конфиг nginx для nginx_exporter

alertmanager.yml пустой так как нет алертов и правил на алерты